...
namespace ZeldaGame.HUD.Menu.InventoryMenuItems
{
    class Boomerang : IMenuItem
    {
        ...
        public Boomerang(PlayerInventory inventory, bool playerTwo)
        {
            Inventory = inventory;
            Sprite = MenuFactory.Instance.CreateSprite("InventoryItems", (int)InventoryItem.None);
            location = MenuFactory.Instance.GetPoint("Boomerang");
        ...
        public void Update()
        {
            if (Inventory.ItemAmount(Item.WoodenBoomerang) > 0)
            {
                if (!set)
                {
                    Sprite.Update((int)InventoryItem.WoodenBoomerang);
                    set = true;
                }
            }
            else
