...
namespace ZeldaGame.HUD.Menu.InventoryMenuItems
{
    class Potion : IMenuItem
    {
        ...
        public Potion(PlayerInventory inventory, bool playerTwo)
        {
            Inventory = inventory;
            Sprite = MenuFactory.Instance.CreateSprite("InventoryItems", (int)InventoryItem.None);
            location = MenuFactory.Instance.GetPoint("Potion");
        ...
        public void Update()
        {
            if (Inventory.ItemAmount(Item.LifePotion) > 0)
            {
                if (!set)
                {
                    Sprite.Update((int)InventoryItem.BluePotion);
                    set = true;
                }
            }
            else
