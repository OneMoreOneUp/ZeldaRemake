...
namespace ZeldaGame.HUD.Menu.InventoryMenuItems
{
    class Arrow : IMenuItem
    {
        ...
        public Arrow(PlayerInventory inventory, bool playerTwo)
        {
            Inventory = inventory;
            Sprite = MenuFactory.Instance.CreateSprite("InventoryItems", (int)InventoryItem.None);
            location = MenuFactory.Instance.GetPoint("Arrow");
        ...
        public void Update()
        {
            if (Inventory.ItemAmount(Item.Arrow) > 0)
            {
                if (!set)
                {
                    Sprite.Update((int)InventoryItem.Arrow);
                    set = true;
                }
            }
            else
