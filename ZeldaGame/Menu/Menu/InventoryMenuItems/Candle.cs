...
namespace ZeldaGame.HUD.Menu.InventoryMenuItems
{
    class Candle : IMenuItem
    {
        ...
        public Candle(PlayerInventory inventory, bool playerTwo)
        {
            Inventory = inventory;
            Sprite = MenuFactory.Instance.CreateSprite("InventoryItems", (int)InventoryItem.None);
            location = MenuFactory.Instance.GetPoint("Candle");
        ...
        public void Update()
        {
            if (Inventory.ItemAmount(Item.RedCandle) > 0)
            {
                if (!set)
                {
                    Sprite.Update((int)InventoryItem.RedCandle);
                    set = true;
                }
            }
            else
