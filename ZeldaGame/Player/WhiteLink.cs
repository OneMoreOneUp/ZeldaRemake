...
{
    public class WhiteLink : Link
    {
        public WhiteLink(Point position, AdventurePlayerInventory inventory, PlayerDataManager data, bool pickup = false, IItem item = null)
        {
        ...
        public override void HoldItem(IItem item)
        {
            ...
            {
                PlayerManager.RemovePlayer(this);
                PlayerManager.AddPlayer(new RedLink(position, inventory, data, true, item));
            }
            else
            {
                this.state = new LinkPickUpItem(this, item, new ItemDataManager(data.GetXPath()), 2);
            }
        }
    }
}
